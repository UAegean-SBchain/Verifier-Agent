...
//    protected ParameterService paramServ = new ParameterServiceImpl();
    private static final Logger LOG = LoggerFactory.getLogger(AfterSSIContactAuthenticator.class);
    ...
    public void authenticate(AuthenticationFlowContext context) {
        try {
            ...
            LOG.info("looking for: " + "claims" + sessionId);
            String claims = (String) mcc.get("claims" + sessionId);
            ...
            if (vc.getContact() != null) {
                ...
                user.setSingleAttribute("email", vc.getContact().getClaims().getEmail());
                user.setSingleAttribute("iban", vc.getContact().getClaims().getIban());
                user.setSingleAttribute("landline", vc.getContact().getClaims().getLandline());
                user.setSingleAttribute("mobilePhone", vc.getContact().getClaims().getMobile());
                user.setSingleAttribute("contact-credential-id", vc.getContact().getMetadata().getId());
                ...
                user.setSingleAttribute("contact-name", credential.getContact().getClaims().getName());
                user.setSingleAttribute("contact-surname", credential.getContact().getClaims().getSurname());
                user.setSingleAttribute("contact-endorser", credential.getContact().getMetadata().getEndorser());
                user.setSingleAttribute("contact-verification", credential.getContact().getMetadata().getVerification());
                user.setSingleAttribute("contact-source", credential.getContact().getMetadata().getSource());
