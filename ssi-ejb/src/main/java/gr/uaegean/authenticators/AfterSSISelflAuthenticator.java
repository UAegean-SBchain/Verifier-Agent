    ...
    public void authenticate(AuthenticationFlowContext context) {
        try {
            ...
            if (vc.getDeclaration() != null) {
                user.setFirstName(vc.getDeclaration().getClaims().getOAEDId());
                user.setLastName(vc.getDeclaration().getClaims().getOAEDRegDate());
                user.setEmail(credential.getDid() + vc.getDeclaration().getClaims().getOAEDId() + "@uport");
                user.setEmailVerified(true);

                user.setSingleAttribute("self-hospitalized", vc.getDeclaration().getClaims().getHospitalized());
                user.setSingleAttribute("self-hospitalizedSpecific", vc.getDeclaration().getClaims().getHospitalizedSpecific());
                user.setSingleAttribute("self-monk", vc.getDeclaration().getClaims().getMonk());
                user.setSingleAttribute("self-luxury", vc.getDeclaration().getClaims().getLuxury());
                user.setSingleAttribute("self-employed", vc.getDeclaration().getClaims().getEmployed());
                user.setSingleAttribute("self-employmentStatus", vc.getDeclaration().getClaims().getEmplymentStatus());
                user.setSingleAttribute("self-oaedid", vc.getDeclaration().getClaims().getOAEDId());
                user.setSingleAttribute("self-oaedDate", vc.getDeclaration().getClaims().getOAEDRegDate());
                user.setSingleAttribute("self-participateFead", vc.getDeclaration().getClaims().getFEADParticipate());
                user.setSingleAttribute("self-feadProvider", vc.getDeclaration().getClaims().getFeadProvider());
                user.setSingleAttribute("credential-id", vc.getDeclaration().getMetadata().getId());
                user.setSingleAttribute("iat", credential.getVerified()[0].getIat());
                user.setSingleAttribute("exp", credential.getVerified()[0].getExp());
                user.setSingleAttribute("credential-name", "KEA_PERSONAL_DECLARATION");
